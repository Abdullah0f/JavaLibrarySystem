...
public class Book {
    ...
    try {
        ...
        while (rs.next()) {
            Object[] row = {rs.getInt(1), rs.getString(2), rs.getInt(3) ,rs.getInt(4) ,rs.getDate(5)};
    ...
    public static void insert(String name, int genreId, int authorId, Date publishDate) {
        DB db = new DB();
        String query = "INSERT INTO library.books (name, genreId, authorId, publish_date) VALUES (?, ?, ?, ?)";
    ...
    public static void update(int id, String name, int genreId, int authorId, Date publishDate) {
        DB db = new DB();
        String query = "UPDATE library.books SET name = ?, genreId = ?, authorId = ?, publish_date = ? WHERE id = ?";
        try (Connection conn = db.getConnection();
                PreparedStatement pstmt = conn.prepareStatement(query)) {
            pstmt.setString(1, name);
            pstmt.setInt(2, genreId);
